### Links

- Solution URL: [Solution URL here](https://github.com/Isuru-Maduwansha/responsive-webpage)
- Live Site URL: [Live site URL here](https://newsletter-sign-up-iota-gold.vercel.app/)

### Built with

- Semantic HTML5 markup
- CSS custom properties
- Flexbox
- Mobile-first workflow
- javaScript

### what i learn

- Building responsive designs for mobile, tablet, and desktop
- Creating responsive layouts using Flexbox and media queries
- Implementing a navbar with a navbar toggler for mobile view

## Author

- Frontend Mentor - [@Isuru-Maduwansha](https://www.frontendmentor.io/profile/Isuru-Maduwansha)
- Github - [@Isuru-Maduwansha](https://github.com/Isuru-Maduwansha)